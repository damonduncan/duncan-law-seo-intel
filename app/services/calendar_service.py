...
EXCLUDE = re.compile(
    ...
    r'|\bsit in\b|\bhold for\b',
    re.IGNORECASE
)
INCLUDE = re.compile(r'consult|appointment booked', re.IGNORECASE)
...
def _is_valid_event(ev: dict, attorney: str) -> bool:
    ...
    # Must start between 8am and 6pm Eastern
    start = ev.get("start", {})
...
def fetch_month_count(email: str, attorney: str, year: int, month: int) -> int:
    """Return filtered consultation count for one attorney/month via Calendar API."""
    from zoneinfo import ZoneInfo
    eastern = ZoneInfo("America/New_York")
    ...
    while True:
        ...
        for ev in result.get("items", []):
            ...
            if eid in seen:
                continue
            seen.add(eid)
            if _is_valid_event(ev, attorney):
                count += 1
    ...
    logger.info(f"Calendar pull {email} {year}-{month:02d}: {count} consultations")
    return count
